...
*/
	...
	memcpy ((unsigned char *)&context->buffer[index], &input[i], inputLen - i);
}
...
void ext_md5calc(unsigned char output[16], const void *input, unsigned int length)
{
	MD5_CTX md5ctx;
	MD5Init(&md5ctx);
	MD5Update(&md5ctx, (const unsigned char *)input, (size_t)length);
